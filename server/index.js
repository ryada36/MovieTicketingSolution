...
app.post('/auth/register',(req,res)=>{
    ...
    if(!body.role){
        ...
        Roles.find({}).then((role)=>{
            if(!role.roles.includes(body.role))
...
app.post('/auth/login',(req,res)=>{
 ...
 app.get('/movies',(req,res)=>{
    ...
    Movie.find({ $and :  [{ releaseDate: { $lte: sevenDaysAfter }}, 
 ...
 app.post('/movies', authenticate,  (req,res)=>{
     ...
     upload(req,res,function(err){
        ...
        let body = _.pick(req.body, ['name','description','duration','releaseDate']);
 ...
 app.get('/theaters/:movieName',(req,res)=>{
    ...
    Movie.find({name: movieName}).then((movies) =>{
        ...
        Show.find({movieId : new ObjectID(movie._id)}).then((shows)=>{
            return new Promise((resolve,reject)=>{
            ...
            Theater.findById(show.theaterId).then((theater)=>{
                if(!theater)
